...
void main(){
  ...
  // Invert the intensity so smoke is high alpha, background is low alpha
  float intensity = 1.0 - dot(col, vec3(.21,.71,.07));
  
  // Make the smoke slightly more ethereal
  float alpha = smoothstep(0.1, 0.8, intensity) * 0.6;
  
  O = vec4(u_color, alpha * min(time*.1, 1.0));
}`;
